...
graph TB
    ...
    subgraph L3["第 3 层：实时操作系统层"]
        TX["ThreadX RTOS"]
        FX["FileX 文件系统"]
    end

    subgraph L2["第 2 层：板级支持层"]
        BSP["BSP Drivers"]
        HAL["STM32 HAL"]
    end

    subgraph L1["第 1 层：硬件层"]
        HW["STM32F407VGT6"]
    end
...
| 层级 | 名称 | 职责 |
|------|------|------|
| **硬件层** | 硬件层 | STM32F407VGT6 MCU、外设：CRC, IWDG, WWDG, MPU, Flash, GPIO |
| **驱动层** | 驱动层 | STM32Cube HAL 驱动、硬件抽象，提供统一 API |
| **系统层** | 系统层 | Azure RTOS ThreadX 6.1.10、多线程调度、同步原语、内存管理 |
| **安全层** | 安全层 | 功能安全核心模块、自检、监控、保护机制 |
| **服务层** | 服务层 | 参数管理服务、诊断服务 |
| **应用层** | 应用层 | 业务逻辑实现、应用线程 |
...
graph LR
    subgraph MPU_Regions["MPU 保护区域"]
        R0["区域 0<br/>Flash (只读+执行)"]
        R1["区域 1<br/>SRAM (读写+禁执行)"]
        R2["区域 2<br/>CCM (读写+禁执行)"]
        R3["区域 3<br/>外设 (读写+禁执行)"]
        R4["区域 4<br/>栈保护 (禁止访问)"]
    end
...
| 区域 | 起始地址 | 大小 | 权限 | 用途 |
|--------|----------|------|------|------|
| 0 | 0x08010000 | 512KB | 只读+执行 | 应用程序 Flash |
| 1 | 0x20000000 | 128KB | 读写 | 主 RAM |
| 2 | 0x10000000 | 64KB | 读写 | CCM RAM (栈) |
| 3 | 0x40000000 | 512MB | 读写+设备 | 外设 |
| 4 | 0x0800C000 | 16KB | 只读 | 配置 Flash |
| 5 | 0x08000000 | 64KB | 禁止访问 | Bootloader (保护) |
...
graph TB
    subgraph Priority1["优先级 1 (最高)"]
        SAFETY_THREAD["安全监控线程<br/>Safety Monitor<br/>栈: 2KB"]
    end

    subgraph Priority2["优先级 2"]
        COMM_THREAD["通信线程<br/>Communication<br/>栈: 4KB"]
    end

    subgraph Priority3["优先级 3"]
        APP_THREAD["应用线程<br/>Application<br/>栈: 4KB"]
    end

    subgraph Priority10["优先级 10 (最低)"]
        IDLE_THREAD["空闲线程<br/>Idle<br/>空闲处理"]
    end
...
| 线程 | 优先级 | 栈大小 | 周期 | 职责 |
|------|--------|--------|------|------|
| 安全监控线程 | 1 (最高) | 2KB | 100ms | 安全监控 |
| 主应用线程 | 5 | 4KB | 10ms | 主业务逻辑 |
| 通信线程 | 10 | 2KB | 事件驱动 | 通信处理 |

### 4.2 线程交互

```mermaid
sequenceDiagram
    participant Safety as 安全线程
    participant App as 应用线程
    participant WDG as 看门狗
...
flowchart TB
    ...
    CPU -->|通过| RAM[RAM March-C 测试]
    CPU -->|失败| SAFE_STATE[进入安全状态]

    RAM -->|通过| FLASH[Flash CRC 验证]
    RAM -->|失败| SAFE_STATE

    FLASH -->|通过| CLOCK[时钟频率检测]
    FLASH -->|失败| SAFE_STATE
...
stateDiagram-v2
    ...
    STARTUP_TEST --> SAFE: 测试失败

    NORMAL --> DEGRADED: 非关键错误
    NORMAL --> SAFE: 关键错误

    DEGRADED --> NORMAL: 错误清除
    DEGRADED --> SAFE: 超时 (30s)
    DEGRADED --> SAFE: 关键错误
...
stateDiagram-v2
    ...
    BOOT_INIT --> BOOT_SELFTEST: 硬件初始化完成
    BOOT_SELFTEST --> BOOT_VERIFY: 测试通过
    BOOT_SELFTEST --> BOOT_FACTORY: 测试失败

    BOOT_VERIFY --> BOOT_JUMP: 应用程序有效
    BOOT_VERIFY --> BOOT_UPDATE: 应用程序无效
    BOOT_VERIFY --> BOOT_FACTORY: 无应用程序

    BOOT_UPDATE --> BOOT_VERIFY: 更新完成
    BOOT_UPDATE --> BOOT_FACTORY: 更新失败

    BOOT_JUMP --> [*]: 跳转到应用程序

    BOOT_FACTORY --> BOOT_FACTORY: 等待恢复
```
...
| 版本 | 日期 | 描述 |
|------|------|------|
| 1.0.0 | 2025-12-10 | 初始版本 (ASCII 图表) |
| 1.0.1 | 2025-12-10 | 整合 Mermaid 图表，删除冗余文件 |
