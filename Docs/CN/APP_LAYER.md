...
graph TB
    subgraph Application["应用层"]
        MAIN[应用主线程<br/>优先级 5]
        COMM[应用通信线程<br/>优先级 10]
    end

    subgraph Safety["安全层"]
        MONITOR[安全监控<br/>优先级 1]
        WDG[看门狗模块]
        STACK[栈监控]
        FLOW[程序流监控]
    end

    subgraph RTOS["ThreadX 实时操作系统"]
        KERNEL[内核调度器]
        POOL[字节池]
    end
...
sequenceDiagram
    ...
    TX->>App: 创建主线程
    TX->>Safety: Stack_RegisterThread(主线程)
    TX->>App: 创建通信线程
    TX->>Safety: Stack_RegisterThread(通信线程)

    Note over TX,App: 内核运行中

    par 并发线程
        Safety->>Safety: 安全监控 [优先级1]
        App->>App: 应用主线程 [优先级5]
        App->>App: 应用通信线程 [优先级10]
    end
```
